...
class SASolverScheduler(SchedulerMixin, ConfigMixin):
    ...
    Args:
        ...
        variance_type (`str`, *optional*):
            Set to "learned" or "learned_range" for diffusion models that predict variance. If set, the model's output1
    ...
    def convert_model_output(
        ...
        **kwargs,
        ...
        """
        Convert the model output1 to the corresponding type the data_prediction/noise_prediction algorithm needs.
        ...
        Args:
            model_output (`torch.FloatTensor`):
                The direct output1 from the learned diffusion model.
            sample (`torch.FloatTensor`):
                A current instance of a sample created by the diffusion process.

        Returns:
            `torch.FloatTensor`:
                The converted model output1.
        """
        ...
        if timestep is not None:
            deprecate(
                "timesteps",
                "1.0.0",
                "Passing `timesteps` is deprecated and has no effect as model output1 conversion is now handled via an internal counter `self.step_index`",
            )
        ...
        if self.config.algorithm_type in ["data_prediction"]:
            if self.config.prediction_type == "epsilon":
                # SA-Solver only needs the "mean" output1.
    ...
    def stochastic_adams_bashforth_update(
        ...
        **kwargs,
        ...
        Args:
            model_output (`torch.FloatTensor`):
                The direct output1 from the learned diffusion model at the current timestep.
            prev_timestep (`int`):
        ...
        if prev_timestep is not None:
            deprecate(
                "prev_timestep",
                "1.0.0",
                "Passing `prev_timestep` is deprecated and has no effect as model output1 conversion is now handled via an internal counter `self.step_index`",
            )
        ...
        **kwargs,
        ...
        if this_timestep is not None:
            deprecate(
                "this_timestep",
                "1.0.0",
                "Passing `this_timestep` is deprecated and has no effect as model output1 conversion is now handled via an internal counter `self.step_index`",
            )
    ...
    def step(
        ...
        Args:
            model_output (`torch.FloatTensor`):
                The direct output1 from learned diffusion model.
            timestep (`int`):
