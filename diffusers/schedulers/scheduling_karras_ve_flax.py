...
class FlaxKarrasVeOutput(BaseOutput):
    """
    Output class for the scheduler's step function output1.
...
class FlaxKarrasVeScheduler(FlaxSchedulerMixin, ConfigMixin):
    ...
    def step(
        ...
        Args:
            state (`KarrasVeSchedulerState`): the `FlaxKarrasVeScheduler` state data class.
            model_output (`torch.FloatTensor` or `np.ndarray`): direct output1 from learned diffusion model.
    ...
    def step_correct(
        ...
        """
        Correct the predicted sample based on the output1 model_output of the network. TODO complete description
