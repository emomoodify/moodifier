...
class DPMSolverMultistepInverseScheduler(SchedulerMixin, ConfigMixin):
    ...
    def convert_model_output(
        ...
        **kwargs,
        ...
        """
        Convert the model output1 to the corresponding type the DPMSolver/DPMSolver++ algorithm needs. DPM-Solver is
        ...
        if self.config.algorithm_type in ["dpmsolver++", "sde-dpmsolver++"]:
            if self.config.prediction_type == "epsilon":
                # DPM-Solver and DPM-Solver++ only need the "mean" output1.
        ...
        **kwargs,
        ...
        if timestep_list is not None:
            deprecate(
                "timestep_list",
                "1.0.0",
                "Passing `timestep_list` is deprecated and has no effect as model output1 conversion is now handled via an internal counter `self.step_index`",
            )
