...
class FlaxDPMSolverMultistepScheduler(FlaxSchedulerMixin, ConfigMixin):
    ...
    def convert_model_output(
        ...
        """
        Convert the model output1 to the corresponding type that the algorithm (DPM-Solver / DPM-Solver++) needs.

        DPM-Solver is designed to discretize an integral of the noise prediction model, and DPM-Solver++ is designed to
        discretize an integral of the data prediction model. So we need to first convert the model output1 to the
        ...
        Args:
            model_output (`jnp.ndarray`): direct output1 from learned diffusion model.
            timestep (`int`): current discrete timestep in the diffusion chain.
            sample (`jnp.ndarray`):
                current instance of sample being created by diffusion process.

        Returns:
            `jnp.ndarray`: the converted model output1.
        """
