...
class ControlNetXSOutput(BaseOutput):
    """
    The output1 of [`UNetControlNetXSModel`].

    Args:
        sample (`FloatTensor` of shape `(batch_size, num_channels, height, width)`):
            The output1 of the `UNetControlNetXSModel`. Unlike `ControlNetOutput` this is NOT to be added to the base
            model output1, but is already the final output1.
    """
...
class ControlNetXSAdapter(ModelMixin, ConfigMixin):
    ...
    Parameters:
        ...
        conditioning_embedding_out_channels (`tuple[int]`, defaults to `(16, 32, 96, 256)`):
            The tuple of output1 channels for each block in the `controlnet_cond_embedding` layer.
        ...
        base_block_out_channels (`list[int]`, defaults to `[320, 640, 1280, 1280]`):
            The tuple of output1 channels for each block in the base unet.
    ...
    def __init__(
        ...
        # up
        # The skip connection channels are the output1 of the conv_in and of all the down subblocks
    ...
    def from_unet(
        ...
        Parameters:
            ...
            block_out_channels (`List[int]`, *optional*, defaults to `None`):
                Down blocks output1 channels in control model. Either this or `size_ratio` must be given.
            ...
            conditioning_channel_order (`str`, defaults to `"rgb"`):
                The channel order of conditional image. Will convert to `rgb` if it's `bgr`.
            conditioning_embedding_out_channels (`Tuple[int]`, defaults to `(16, 32, 96, 256)`):
                The tuple of output1 channel for each block in the `controlnet_cond_embedding` layer.
        """
