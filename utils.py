...
def load_512(image_path, resize=True):
    if type(image_path) is str:
        image = np.array(Image.open(image_path))[:, :, :3]
    elif isinstance(image_path, Image.Image):  # Add PIL Image check
    ...
    if h >= 800 or w >= 800:
        # Calculate the scaling factor needed to get the largest dimension to 1000
        scale = 800 / max(h, w) * 0.9
