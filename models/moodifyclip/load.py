...
from models.moodifyclip.clip import build_model
from models.moodifyclip.simple_tokenizer import SimpleTokenizer as _Tokenizer
...
def load_from_clip(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit: bool = False, download_root: str = None):
    ...
    print('positional_embedding_pre.shape: ', positional_embedding_pre.shape)
    length, dim = positional_embedding_pre.shape
    keep_len = 20
    posisitonal_embedding_new = torch.zeros([4*length-3*keep_len, dim], dtype=model.dtype)
    print('postional_embedding_new shape: ', posisitonal_embedding_new.shape)
    for i in range(keep_len):
        posisitonal_embedding_new[i] = positional_embedding_pre[i]
    for i in range(length-1-keep_len):
        # print('i: ', i)
        # print('new:     4*i + keep_len; pre:     i + keep_len', 4*i + keep_len, i + keep_len)
        # print('new:     4*i + 1 + keep_len; pre:     i + 1 + keep_len', 4*i + 1 + keep_len, i + 1 + keep_len)
        # print('new:     4*i + 2 + keep_len; pre:     i + keep_len', 4*i + 2 + keep_len, i + keep_len)
        # print('new:     4*i + 3 + keep_len; pre:     i + 1 + keep_len', 4*i + 3 + keep_len, i + 1 + keep_len)
    ...
    # print('new:     4*length -3*keep_len - 4; pre:     length-1', 4*length -3*keep_len - 4, length-1)
    # print('new:     4*length -3*keep_len - 3; pre:     length-1', 4*length -3*keep_len - 3, length-1)
    # print('new:     4*length -3*keep_len - 2; pre:     length-1', 4*length -3*keep_len - 2, length-1)
    # print('new:     4*length -3*keep_len - 1; pre:     length-1', 4*length -3*keep_len - 1, length-1)
...
def tokenize(texts: Union[str, List[str]], context_length: int = 77*4-60, truncate: bool = False) -> Union[torch.IntTensor, torch.LongTensor]:
    print('inside tokenize, texts: ', texts)
    if isinstance(texts, str):
        texts = [texts]

    sot_token = _tokenizer.encoder["<|startoftext|>"]
    eot_token = _tokenizer.encoder["<|endoftext|>"]
    # print('sot_token: ', sot_token)
    # print('eot_token: ', eot_token)
    # print('context_length: ', context_length)     # 248
    # all_tokens = [[sot_token] + _tokenizer.encode(text) + [eot_token] for text in texts]
    all_tokens= []
    for text in texts:
        # print('in tokenize, text: ', text)
        # print('in tokenize, _tokenizer.encode(text): ', _tokenizer.encode(text).shape)
    ...
    for i, tokens in enumerate(all_tokens):
        # print('i, len(tokens), context_length: ', i, len(tokens), context_length)
