...
def p2p_guidance_forward(
    ...
    text_input_ids = model.tokenizer(prompt)
    text_embeddings = model.text_encoder(text_input_ids.to(model.device))
    uncond_input_ids = model.tokenizer([""] * batch_size)
    uncond_embeddings_ = model.text_encoder(uncond_input_ids.to(model.device))
...
def direct_inversion_p2p_guidance_forward(
    ...
    text_embeddings = model.text_encoder(text_input_ids.to(model.device))
    uncond_input_ids = model.tokenizer([""] * batch_size)
    uncond_embeddings = model.text_encoder(uncond_input_ids.to(model.device))
