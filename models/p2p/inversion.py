...
class NullInversion:
    ...
    def init_prompt(self, prompt: str):
        ...
        # )
        # text_embeddings = self.model.text_encoder(text_input.input_ids.to(self.model.device))[0]
        text_input_ids = self.model.tokenizer(prompt)
        uncond_text_input_ids = self.model.tokenizer([""])
        text_embeddings = self.model.text_encoder(text_input_ids.to(self.model.device))
        uncond_embeddings = self.model.text_encoder(uncond_text_input_ids.to(self.model.device))
        self.context = torch.cat([uncond_embeddings, text_embeddings])
        self.prompt = prompt
...
class DirectInversion:
    ...
    def init_prompt(self, prompt: str):
        ...
        # text_embeddings = self.model.text_encoder(text_input.input_ids.to(self.model.device))[0]
        text_input_ids = self.model.tokenizer(prompt)
        uncond_text_input_ids = self.model.tokenizer([""]*len(prompt))
        text_embeddings = self.model.text_encoder(text_input_ids.to(self.model.device))
        uncond_embeddings = self.model.text_encoder(uncond_text_input_ids.to(self.model.device))
