...
class LocalBlend:
    ...
    def __call__(self, x_t, attention_store):
        ...
        if self.counter > self.start_blend:
            ...
            combined_latent_binary_mask = torch.zeros_like(mask, dtype=torch.float32)

            for emotion_stimuli_mask, importance, targeted_stimuli in (
                    zip(self.emotion_stimuli_masks,
                        self.importance_scores,
                        self.emotion_stimuli_objs)):
                binary_mask = load_binary_mask(emotion_stimuli_mask)
                latent_binary = F.interpolate(
                    binary_mask.float(),
                    size=(x_t.shape[-2], x_t.shape[-1]),
                    mode='bilinear',
                    align_corners=False
                )
                latent_binary = latent_binary.to(x_t.device)
                combined_latent_binary_mask += latent_binary.mean(dim=1, keepdim=True)[0]
            mask = combined_latent_binary_mask
    ...
    def __init__(self, prompts, words, substruct_words=None, start_blend=0.2, th=(.7, .7),
                 tokenizer=None, device=None,num_ddim_steps=50, emotion_stimuli_objs=None, emotion_stimuli_masks=None,
...
def make_controller(pipeline,
                    ...
                    num_ddim_steps=50,
                    device=None,
                    emotion_stimulus_lst=None,
                    emotion_stimuli_masks=None,
                    importance_scores=None) -> AttentionControlEdit:
    if blend_words is None:
        lb = None
    else:
        lb = LocalBlend(prompts, blend_words, tokenizer=pipeline.tokenizer, device=device,num_ddim_steps=num_ddim_steps,
                        emotion_stimuli_objs=emotion_stimulus_lst,
